...
# 创建测试类
from utils_工具类 import assert_common, read_emp_data
...
class TestIHRMEmployeeParams(unittest.TestCase):
    ...
    # 定义员工模块的文件路径
    emp_filepath = app_配置信息.BASE_DIR + "/json_data/emp_data_json.json"
    ...
    def test04_modify_emp(self, username, success, code, message, http_code):
        # 调用封装的修改员工接口，发送接口请求
        response = self.emp_api.modify_emp(app_配置信息.EMP_ID, {"username": username}, app_配置信息.HEADERS)
        # 打印数据
